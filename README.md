...
shows `IOB: 5.2 U (eIOB: 1.7 U)`. Both switches default to off;
...
Rows with an additional note expand to show that note. Rows without extra
details are plain text. A chart click highlights the matching visible row and
opens its note, if present. The separate details area shows the full entry when
the list is hidden, collapsed, or that entry is outside the displayed list.
Click the chart pill again to clear the selection. The drag cursor appears only when the chart is zoomed in.
